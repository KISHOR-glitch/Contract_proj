...
public interface WorkflowHistoryRepository extends JpaRepository<WorkflowHistory, UUID> {
    ...
    /**
     * Returns all workflow transitions for a given contract,
     * ordered chronologically ascending (oldest first).
     */
    List<WorkflowHistory> findByContractIdOrderByChangedAtAsc(UUID contractId);
}
